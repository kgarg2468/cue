...
**Addendum (cycle 18, post delta re-review):** Sol's delta re-review confirmed the blocker and majors #1/#5/#7 fixed and accepted the deferrals above, leaving two residuals now also recorded as deliberate checkpoint tradeoffs: (8) on a save failure the app renames the private plaintext temp recording with a `held-` prefix (still in the 0700 owner-only directory), and next-launch reconciliation always preserves `held-` files — so quitting with the failure unresolved keeps the only copy of the user's audio on disk. What this slice does *not* do: recover that file into the app UI on next launch (no in-memory pending state survives quit), and a crash while recording or mid-save (before the failure sheet appears) still leaves a process-owned temp that reconciliation deletes. The M2 persistence work should add a durable pending-capture manifest plus a launch-time recovery flow so both cases round-trip losslessly; (9) persistence-mutation failures surface via reload plus `loadErrorMessage`, currently rendered only on the Today view — M2 UI work should surface store errors on Moments and detail surfaces too. The remaining copy-honesty instances Sol flagged (explainer, Today card, usage description) were fixed in this slice, as was the new minor #15 (capture started during the 400 ms retry-routing window could be navigated away; `isSaving` now holds until routing completes).
