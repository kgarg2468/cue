...
| M0-FOUNDATION-001 | Approved M0 plan | Swift macOS application and IPC client shell | M0 | `Package.swift`; `Sources/CaptureDelegateApp/`; `Sources/CaptureDelegateIPC/` | Post-merge `scripts/verify-app-launch.sh` failed 10/10 consecutive launches: LaunchServices/System Events reported CaptureDelegateApp foreground and visible with zero windows, and CoreGraphics reported no window. Tested and reverted AppDelegate activation, a single SwiftUI Window scene, and an explicit AppKit-owned NSWindow. | PR #1 (`3e6cf85`) | blocked |
