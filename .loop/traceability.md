...
## UI-bearing slice UX gate

For every UI-bearing slice, its traceability/PR evidence must identify the exact tested commit and packaged `.app` build, named scenario states, Computer Use result and `.loop/verification/<milestone>/<slice>/ux/` path, Opus UX-contract and user-facing SwiftUI implementation provenance, a fresh independent Opus visual review, independent gpt-5.6-sol technical review, and `user_verdict: pending|approved|rejected`. The UX directory retains screenshots, AX trees, scenario log, real-data/backend provenance, and the Opus review. Computer Use must drive the packaged `.app`; process/window-count smoke, unit tests, previews, and mocks are supporting checks and cannot close product verification. A UI PR pauses before merge while pending; rejection reopens the same milestone/slice. Opus or Computer Use unavailability is `blocked`, never a downgrade. Internal-only fixes may unblock a slice but do not advance product progress. ADR-007 local parity and Sol review remain mandatory; GitHub Actions is not a gate.
...
| S02-INC-01 | 2 Included | Capture intentional macOS audio from microphone, system audio, or both | M1.5 (mic) + M3 (remaining sources) | — | — | — | todo |
...
| S02-INC-05 | 2 Included | Support personal voice notes and thinking aloud | M1.5 | — | — | — | todo |
...
| S02-EXC-02 | 2 Excluded | Do not permit secret, invisible, or always-on recording | M1.5 + M3 | — | — | — | todo |
...
| S04-002 | 4.2 | Start recording only by explicit button, menu action, shortcut, or supported calendar trigger and visibly indicate active state | M1.5 + M3 | — | — | — | todo |
...
| S04-006 | 4.6 | Perform recording, transcription, storage, and basic extraction locally by default; cloud is optional | M1.5 + M4 + M12 | — | — | — | todo |
...
| S05-001 | 5 Session | Model intentional session types: meeting, conversation, presentation, pair work, personal note, imported audio | M1.5 (minimum capture session) + M2 | — | — | — | todo |
| S05-002 | 5 Session | Persist session audio sources, transcript, speakers, markers, attachments, note, actions, sources, projects, and runs | M1.5 (session/audio) + M2 | — | — | — | todo |
...
| S06-001 | 6.1 | Start session from menu bar, main app, shortcut, or calendar prompt | M1.5 (main app) + M3 | — | — | — | todo |
...
| S06-003 | 6.1 | Display recording state, elapsed time, active sources, transcript status, privacy mode, action count, and running agents | M1.5 (capture state) + M3–M7 | — | — | — | todo |
...
| S06-009 | 6.4 | Navigate to Session Detail rather than agent dashboard after capture | M1.5 | — | — | — | todo |
...
| S10-001 | 10 | Local helper launches processes/PTYS, worktrees, enforces limits, streams stdout/stderr/events, detects waiting input, redacts logs, records metadata, terminates/culls resources | M1 | `backend/src/lib.rs` and `Sources/CaptureDelegateIPC/IPCClient.swift`: bounded pipe/PTY execution and event streaming; typed timeout/cancel/pause/resume/stdin controls; waiting-input detection; redacted terminal metadata; process-tree and coordinated shutdown cleanup; isolated private worktrees with bounded Git cleanup; torn-frame-safe terminal durability; atomic 1 MiB pending-stdin admission; private PID owner sidecars and startup recovery of only authenticated dead-owner managed worktrees | `.loop/verification/m1/process-streaming/`; `.loop/verification/m1/run-timeout/`; `.loop/verification/m1/run-cancel/`; `.loop/verification/m1/process-groups/`; `.loop/verification/m1/shutdown-cleanup/`; `.loop/verification/m1/pause-resume/`; `.loop/verification/m1/run-stdin/`; `.loop/verification/m1/run-metadata/`; `.loop/verification/m1/pty/`; `.loop/verification/m1/input-wait/`; `.loop/verification/m1/worktree/`; `.loop/verification/m1/durability/`; `.loop/verification/m1/resource-safety/verification.txt` | PRs #5 (`9f40024`), #7 (`3058370`), #9 (`ea02d67`), #11 (`8877242`), #13 (`af7db55`), #15 (`9291f52`), #17 (`67d39f2`), #19 (`9c36a5f`), #21 (`8e58f7d`), #23 (`1ad2785`), #25 (`2a5406b`), #27 (`36d3705`), and #29 (`6f7d647`; ADR-007 parity green; initial Sol MAJOR fixed; final Sol and Opus PASS) | verified |
...
| S11-001 | 11 | Core recorder/local-agent flow works without cloud; cloud is optional | M1.5 (recorder) + M7 (local agent) + M12 | — | — | — | todo |
...
| S14-001 | 14 Privacy | Default to local raw audio/transcription/encryption, Keychain keys, no account/model training, per-session deletion/retention | M1.5 (encrypted audio/Keychain) + M4 + M10 | — | — | — | todo |
| S14-002 | 14 Transparency | Provide persistent/menu indicators, optional sound, source labels, pause, exclusions, private mode | M1.5 (visible mic/pause) + M3 | — | — | — | todo |
...
| S18-001 | 18.1 | UI is calm/native/keyboard fluent, note-first and not enterprise orchestration; every screen answers captured, became, happening | Every UI milestone + M11 cohesion | — | — | — | todo |
| S18-002 | 18.2 | Provide five surfaces: menu popover, floating HUD, main window, dispatch sheet, notifications | M1.5 + M3 + M6 + M10 | — | — | — | todo |
| S18-003 | 18.3 | Main window default/minimum sizes are 1180×760/920×620; collapsible sidebar has Today/Moments/Projects/Actions/Runs/Search/pins | M1.5 + M6–M8 | — | — | — | todo |
| S18-004 | 18.3 | Toolbar has sidebar, title, global search, New Capture, palette, account/sync, compact run pill | M1.5 + M7 + M8 + M12 | — | — | — | todo |
| S18-005 | 18.4 | Menu idle shows start, quick profiles, recents, agents, main/settings; recording shows status, markers, pause/stop; request state handles permission | M1.5 + M3 + M7 + M10 | — | — | — | todo |
| S18-006 | 18.4 | Menu popover supports full keyboard navigation | M1.5 + M3 | — | — | — | todo |
| S18-007 | 18.5 | Movable 360×56 HUD shows recording/source/controls and expanded transcript/action view | M3 + M4 + M6 | — | — | — | todo |
| S18-008 | 18.5 | HUD stays above normal windows unless disabled, avoids controls, collapses, always signals recording, hides text on shared screens when enabled | M3 | — | — | — | todo |
| S18-009 | 18.6 | Today is calm overview with Start capture, Needs you, Ready, Recent, Running; empty sections collapse | M1.5 + M6 + M7 | — | — | — | todo |
| S18-010 | 18.7 | Capture setup offers title, audio choices, active apps/screenshots/repo context, local/retention privacy, optional purpose without required categorization | M1.5 (minimum mic) + M3 | — | — | — | todo |
| S18-011 | 18.8 | Optional live window has transcript, detected rail, controls/direct field; explicit dispatch visibly starts, suggestions do not focus-steal, edits do not stop capture | M4 + M6 | — | — | — | todo |
| S18-012 | 18.9 | Session Detail is primary reader: header, Note/Transcript/Sources default Note, Actions/Context/Info inspector | M1.5 (minimum detail/playback) + M5 + M6 | — | — | — | todo |
| S18-013 | 18.9 | Notes offer source count/jump/edit/incorrect/action/artifact; transcript has speaker gutter/timestamps/confidence/playback/selection/context menu; actions show resolution state | M1.5 (audio playback) + M5 + M6 | — | — | — | todo |
| S18-014 | 18.10 | Compact/expanded action cards show objective, source/playback, project, deliverable, access and Copy/Open/Run/Edit controls | M6 | — | — | — | todo |
| S18-015 | 18.10 | Statuses Draft, Needs review, Ready, Queued, Running, Needs you, Complete, Failed, Dismissed use text and icon not color alone | M6 | — | — | — | todo |
| S18-016 | 18.11 | Dispatch sheet selects agent/mode/capability, plain-language allow/deny, local/hosted, time/cost; Build shows isolated worktree/diff/push/PR explicit grants | M6 + M7 + M9 + M12 | — | — | — | todo |
| S18-017 | 18.12 | Actions queue groups Needs you/Ready/Running/Completed; filters status/project/agent/capability/session/speaker; bulk action limits and confirmation | M6 | — | — | — | todo |
| S18-018 | 18.13 | Runs screen provides list/control center, progress/live/context/permissions/artifacts, pause/stop/terminal, permission/clarification/complete result; post review creates new Act action | M7 + M9 | — | — | — | todo |
| S18-019 | 18.14 | Project screen is lightweight, shows objective/questions/actions/artifacts/moments/resources, and proposes updates rather than manual pages | M8 | — | — | — | todo |
| S18-020 | 18.15 | ⌘K palette supports listed capture/mark/send/open/show/create/search/switch commands and natural-language resolution | M3 + M6–M9 | — | — | — | todo |
| S18-021 | 18.16 | Provide customizable defaults for Alt+Space, Alt+Shift+Space, Alt+A/D/I/Q, ⌘K, ⌘Return, ⌘Shift+Return, Space, ⌘L | M3–M7 | — | — | — | todo |
...
| S18-025 | 18.20 | Design is calm/editorial/native/trustworthy/minimal; system type/monospace technical values; text/icon color semantics; subtle/reduced motion/no layout shift; SF Symbols and explicit permission icons | Every UI milestone + M11 cohesion | — | — | — | todo |
...
| S18-027 | 18.22 | Provide specified No sessions, No agents, No actions, nonblocking Processing, and Low-confidence states | M1.5 + M4 + M6 + M7 | — | — | — | todo |
...
| S10-005 | 10 Responsibilities | Launch child processes and PTYs | M1 | `backend/src/lib.rs` launches direct argv processes with pipes or an opt-in real controlling PTY; `Sources/CaptureDelegateIPC/IPCClient.swift` exposes typed `startProcess` and PTY selection | Pipe and PTY launch, terminal identity, partial-line VEOF, timeout/cancel/redaction/metadata tests; `.loop/verification/m1/process-streaming/`; `.loop/verification/m1/pty/verification.txt` | PR #5 (`9f40024`); PR #21 (`8e58f7d`; ADR-007 gate and independent Sol review) | verified |
| S10-006 | 10 Responsibilities | Create isolated Build worktrees | M7 | — | — | — | todo |
| S10-007 | 10 Responsibilities | Enforce local timeout and concurrency limits | M1 | `backend/src/lib.rs` enforces bounded eight-client/eight-process pools, required per-run timeouts with group kill/reap, typed run-capacity exhaustion, and an atomic 1 MiB per-run pending-stdin cap whose rejection admits no partial bytes | Process admission, timeout, capacity release, u64 boundary, and real paused-stdin cap/retry tests; Swift typed decoding; `.loop/verification/m1/run-timeout/verification.txt`; `.loop/verification/m1/resource-safety/verification.txt` | PR #5 (`9f40024`); PR #7 (`3058370`); PR #29 (`6f7d647`; ADR-007 parity and final independent review PASS) | verified |
| S10-008 | 10 Responsibilities | Stream stdout, stderr, and structured events | M1 | `backend/src/lib.rs` uses bounded concurrent pipe/PTY drains and serialized typed output, waiting-input, metadata, error, and exactly-once terminal events; `Sources/CaptureDelegateIPC/IPCClient.swift` exposes the typed callback stream | Rust lifecycle, UTF-8, slow/non-reading client, ordering, PTY, metadata, and inherited-pipe tests; Swift callback and real-backend integration tests; `.loop/verification/m1/process-streaming/`; `.loop/verification/m1/durability/verification.txt` | PR #5 (`9f40024`); PR #21 (`8e58f7d`); PR #27 (`36d3705`; ADR-007 parity and independent Sol review) | verified |
| S10-009 | 10 Responsibilities | Detect agent waits for input | M1 | Opt-in `input_wait_detect_milliseconds` emits at most one `run_input_waiting` frame per quiet episode when output is idle, stdin is drained, and child CPU is quiescent; it is suppressed while paused, nonblocking under backpressure, and works for pipe and PTY runs; Swift decodes `.inputWaiting` | 7 Rust integration tests including backpressure-cancel and paused-suppression regressions; Swift wire/decode tests; red evidence and runtime demo; `.loop/verification/m1/input-wait/verification.txt` | PR #23 (`1ad2785`; ADR-007 parity green; 1 blocker, 1 major, and 1 minor confirmed and fixed before final independent review) | verified |
| S10-010 | 10 Responsibilities | Redact secrets from logs and record process/environment metadata | M1 | `backend/src/lib.rs`: `redact_output` applies a compiled fixed pattern set (AWS/GitHub/Slack/OpenAI tokens, Bearer, sensitive key=value, PEM blocks) to every `run_output` frame on both streams; `run_metadata` frame (pid, pgid, executable, arguments, working_directory, RFC3339 started/finished, duration_ms, sorted environment variable NAMES only, redaction count) emitted after final output and before `run_exit` on all spawned terminal paths, with progressive truncation under the 8KiB frame cap; spawn failures emit none; `Sources/CaptureDelegateIPC/IPCClient.swift` `.metadata` event. Accepted limitations documented: chunk-split secrets, fixed pattern set, prose over-redaction | `.loop/verification/m1/run-metadata/` (red evidence, cargo 15+13+31, verify-m0 parity pre/post-fix, runtime demos incl. partial/full truncation, review report + dispositions, verify-m0-premerge.log) | PR #19 (`9c36a5f`; CI unavailable — merged under ADR-007 gate: local verify-m0 parity exit 0 + independent Codex sol review, 1 blocker + 2 majors confirmed and fixed pre-merge) | verified |
| S10-011 | 10 Responsibilities | Terminate cancellation/budget-expired runs and clean temporary files/worktrees | M1 | `backend/src/lib.rs` applies timeout/cancel/natural-exit/error and backend-shutdown termination to each run's full process group, releases admission, emits one durable terminal, and synchronously cleans isolated worktrees; private owner sidecars let startup reclaim only provably dead-owner direct managed worktrees, while canonical reciprocal Git metadata and branch-prefix guards prevent unrelated repository/branch deletion | Timeout/cancel/race, descendant-kill, signal-shutdown, pause/resume, worktree terminal cleanup, write-failure durability, real startup orphan cleanup, malformed/symlink/live-owner, and forged cross-repository metadata tests; `.loop/verification/m1/run-timeout/`; `.loop/verification/m1/run-cancel/`; `.loop/verification/m1/process-groups/`; `.loop/verification/m1/shutdown-cleanup/`; `.loop/verification/m1/worktree/`; `.loop/verification/m1/durability/`; `.loop/verification/m1/resource-safety/verification.txt` | PRs #5 (`9f40024`), #7 (`3058370`), #9 (`ea02d67`), #11 (`8877242`), #13 (`af7db55`), #15 (`9291f52`), #25 (`2a5406b`), #27 (`36d3705`), and #29 (`6f7d647`; initial Sol MAJOR fixed; final Sol and Opus PASS) | verified |
...
| S14-006 | 14 Privacy | Store encryption keys in macOS Keychain | M1.5 | — | — | — | todo |
...
| S14-009 | 14 Transparency | Support one-click pause, app exclusion list, and private-session mode | M1.5 (pause) + M3 | — | — | — | todo |
...
| S18-SIDEBAR-01 | 18.3 | Sidebar offers Today, Moments, Projects, Actions, Agent Runs, and Search | M1.5 + M6–M8 | — | — | — | todo |
| S18-SIDEBAR-02 | 18.3 | Sidebar supports pinned projects | M8 | — | — | — | todo |
| S18-HUD-01 | 18.5 | HUD supports Important, Action, Pause, Stop controls | M3 + M6 | — | — | — | todo |
| S18-HUD-02 | 18.5 | Expanded HUD supports Decision and Question markers plus detected action Save/Send | M3 + M4 + M6 | — | — | — | todo |
| S18-SETUP-01 | 18.7 | Capture setup supports microphone+system, microphone-only, and system-only choices | M3 | — | — | — | todo |
| S18-SETUP-02 | 18.7 | Capture setup supports active-app names, marked screenshots, and current-repository context toggles | M3 | — | — | — | todo |
| S18-SETUP-03 | 18.7 | Capture setup supports local raw-audio retention and post-30-day deletion choices | M3 + M10 | — | — | — | todo |
| S18-SETUP-04 | 18.7 | Capture purpose options are Auto, Meeting, Conversation, Presentation, Personal note, Pair work | M3 | — | — | — | todo |
| S18-NOTE-01 | 18.9 | Note default sections include Summary, Decisions, Ideas, and Open questions | M5 | — | — | — | todo |
| S18-TRANS-01 | 18.9 | Transcript playback highlights active text and supports selection-to-action | M5 + M6 | — | — | — | todo |
| S18-TRANS-02 | 18.9 | Transcript context menu marks decision, idea, question, or source | M5 | — | — | — | todo |
| S18-ACTION-01 | 18.10 | Action component exposes Draft, Needs review, Ready, Queued, Running, Needs you, Complete, Failed, Dismissed individually | M6 | — | — | — | todo |
| S18-DISPATCH-01 | 18.11 | Dispatch supports Copy prompt, Open interactive session, and background Run choices | M6 + M7 | — | — | — | todo |
| S18-DISPATCH-02 | 18.11 | Dispatch supports Read, Plan, Build, Act capabilities | M6 + M7 + M9 | — | — | — | todo |
| S18-DISPATCH-03 | 18.11 | Dispatch shows capability plain-language positive and negative permissions | M6 + M9 | — | — | — | todo |
| S18-DISPATCH-04 | 18.11 | Dispatch supports local Mac or hosted worker and maximum time/cost limits | M6 + M12 | — | — | — | todo |
| S18-DISPATCH-05 | 18.11 | Build dispatch supports isolated worktree, show diff, explicit push, explicit open PR | M7 + M9 | — | — | — | todo |
| S18-QUEUE-01 | 18.12 | Queue filter supports status, project, agent, capability, session, source speaker | M6 | — | — | — | todo |
| S18-QUEUE-02 | 18.12 | Safe bulk actions are dismiss, project assign, agent change; autonomous bulk requires summary confirmation | M6 + M9 | — | — | — | todo |
| S18-RUN-01 | 18.13 | Run detail provides Progress, Live output, Context, Permissions, Artifacts tabs | M7 | — | — | — | todo |
| S18-RUN-02 | 18.13 | Run progress shows packet load, PR open, implementation read, test compare, review preparation | M7 | — | — | — | todo |
| S18-RUN-03 | 18.13 | Clarification displays source context and reply field | M7 | — | — | — | todo |
| S18-CMD-01 | 18.15 | Palette starts microphone and microphone+system capture and stops capture | M3 | — | — | — | todo |
| S18-CMD-02 | 18.15 | Palette marks action, sends selected action, opens latest session, shows running agents | M3 + M6 + M7 | — | — | — | todo |
| S18-CMD-03 | 18.15 | Palette creates clipboard action, searches sessions, and switches project | M6 + M8 | — | — | — | todo |
...
| S18-DESIGN-01 | 18.20 | Use macOS system typeface and readable human-note line height/measure | Every UI milestone + M11 | — | — | — | todo |
| S18-DESIGN-02 | 18.20 | Render commands, paths, technical values, packets in monospaced type | Every UI milestone + M11 | — | — | — | todo |
| S18-DESIGN-03 | 18.20 | Color semantics: neutral captured/saved, blue ready, purple running, orange input, green complete, red failure/destructive | Every UI milestone + M11 | — | — | — | todo |
| S18-DESIGN-04 | 18.20 | Use subtle insert motion, only recording pulses, respect Reduce Motion, avoid stream layout shifts | Every UI milestone + M11 | — | — | — | todo |
| S18-DESIGN-05 | 18.20 | Prefer SF Symbols and explicit eye/list/hammer/act/lock/waveform/link permission/provenance icons | Every UI milestone + M11 | — | — | — | todo |
| S18-EMPTY-01 | 18.22 | No-sessions state invites meeting/conversation/presentation/idea capture and Start Capture | M1.5 | — | — | — | todo |
| S18-EMPTY-02 | 18.22 | No-agents state preserves notes and offers connect/not-now | M6 + M7 | — | — | — | todo |
| S18-EMPTY-03 | 18.22 | No-actions state explains manual action creation from transcript selection | M6 | — | — | — | todo |
| S18-EMPTY-04 | 18.22 | Processing permits transcript reading while note refinement continues without blocking spinner | M4 + M5 | — | — | — | todo |
| S19-006 | 19.1 | Work-meeting voice dispatch uses two-second confirmation toast | M6 + M9 | — | — | — | todo |
| S19-007 | 19.2 | Dependent Plan action waits for both parallel research runs and is non-coding | M9 | — | — | — | todo |
| S19-008 | 19.5 | User can review Build diff in app or preferred editor before separate PR Act confirmation | M7 + M9 | — | — | — | todo |
