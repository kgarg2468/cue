...
| S10-011 | 10 Responsibilities | Terminate cancellation/budget-expired runs and clean temporary files/worktrees | M1 | `backend/src/lib.rs` cancels, kills, and reaps an active producer after output disconnect and now kills/reaps direct children on per-run timeout while releasing capacity; public cancel/budget APIs, descendant process groups, and temporary/worktree cleanup remain pending | PR #5 disconnect/inherited-pipe tests; Rust timeout output/order/capacity tests; Swift real-backend timeout test; `.loop/verification/m1/run-timeout/verification.txt`; independent review clear | PR #5 (`9f40024`; both CI runs passed); timeout PR pending | in_progress |
