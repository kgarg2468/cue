...
| S05-001 | 5 Session | Model intentional session types: meeting, conversation, presentation, pair work, personal note, imported audio | M2 | `backend/src/store.rs` (durable Session model: id/title/timestamps; typed session kinds pending) | `.loop/verification/m2-persistence/` (red-evidence, cargo-test, verify-m0 parity ×2, runtime restart demo) | PR #33 (`763d62d`) | in_progress |
