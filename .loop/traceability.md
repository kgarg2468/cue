...
| S02-INC-08 | 2 Included | Support optional project and repository association | M8 | migration 10 `projects` + `session_projects` link table, `create_project`/`link_session_project`/`list_session_projects` (durable association; suggestion/association UX pending M8) | `.loop/verification/m2-projects/` (red-first tests, v9→v10 demo, Sol round-1 PASS) | PR #53 (`5ca9e25`) | in_progress |
...
| S05-001 | 5 Session | Model intentional session types: meeting, conversation, presentation, pair work, personal note, imported audio | M2 | `backend/src/store.rs` (Session.kind, migration 2), `backend/src/lib.rs` (SESSION_KINDS allowlist, optional validated kind), `Sources/CaptureDelegateIPC/IPCClient.swift` (Session.kind) | `.loop/verification/m2-session-kinds/` (red-evidence, cargo-test, verify-m0 ×2, real-binary v1→v2 migration + kind round-trip demo); prior slice evidence in `.loop/verification/m2-persistence/` | PR #33 (`763d62d`), PR #35 (`3690564`) | verified |
| S05-002 | 5 Session | Persist session audio sources, transcript, speakers, markers, attachments, note, actions, sources, projects, and runs | M2 | `backend/src/store.rs` + `create_session`/`list_sessions` protocol (sessions, sources, runs, markers, session note, transcript segments with speakers, detected-action links, and related-project links survive restart via PRs #33/#37/#39/#41/#45/#47/#51/#53; attachments/actions/projects pending) | `.loop/verification/m2-persistence/` (restart-survival E2E `SessionPersistenceTests`, bounded-list tests, verify-m0 parity ×2) | PR #33 (`763d62d`) | in_progress |
| S05-003 | 5 Source reference | Persist stable session ID, start/end ms, speaker, and exact source text | M2 | `backend/src/store.rs` (Source, migration 3), `backend/src/lib.rs` (add_source/list_sources), `Sources/CaptureDelegateIPC/IPCClient.swift` (Source) | `.loop/verification/m2-source-refs/` (red-evidence, cargo-test, verify-m0 ×2, real-binary v2→v3 migration + chronological restart demo) | PR #37 (`d926c62`) | verified |
...
| S05-006 | 5 Action | Model extracted/manual actions as drafts requiring approval or policy before execution | M6 | migration 9 `actions` table + `create_action`/`list_actions` (drafts persist with closed §7.1 taxonomy, optional session link, server-minted draft status; approval/rejection transitions pending M6/M9 policy machinery) | `.loop/verification/m2-actions/` (red-first contract tests, forced-status demo, v8→v9 migration demo, Sol round-1 PASS) | PR #51 (`60dd30a`) | in_progress |
| S05-007 | 5 Task packet | Model structured provider-neutral approved-action packet | M6 | migration 11 `task_packets` table + `create_task_packet`/`list_task_packets` (versioned §8 document stored in canonical JSON, `task_packet_version` pinned to 1, content-byte cap, binding list-envelope probe, float_roundtrip serialization fixed point; packet-schema internals pending M6/M13) | `.loop/verification/m2-task-packets/` (red-first contract tests, mutant-proven probe, v10→11 runtime demo with self-contained fixture, Sol round-1 FAIL→fixed + round-2 PASS) | PR #55 (`3587510`) | in_progress |
| S05-008 | 5 Run | Persist agent, permissions, environment, events, cost, status, result, and artifacts for each run | M2 | migration 4 `runs` table + admission-time insert in `start_process`, one-shot terminal close, startup interruption sweep, canonical-path store-ownership lock, bounded `list_runs`, Swift `RunRecord`/`RunListPage` (executable/status/exit result/session link/lifecycle events persisted via PRs #39/#43/#49; permissions/environment/cost/artifacts pending) | `.loop/verification/m2-run-records/` (red-evidence ×4 rounds, runtime demo v3→v4 + SIGKILL→interrupted, verify-m0 parity ×3, Sol round-5 PASS report) | PR #39 (`35ab350`) | in_progress |
...
| S08-001 | 8 | Version provider-neutral packet with ID/timestamps, origin/source refs, project/revision, action, execution, included/excluded context | M6 | `task_packets` rows carry id, action link, pinned version, creation stamp, and the full document body in canonical JSON (PR #55); the structured origin/project/action/execution/context schema lands with packet building in M6 | `.loop/verification/m2-task-packets/` (§8-shaped document round-trip on real binary) | PR #55 (`3587510`) | in_progress |
