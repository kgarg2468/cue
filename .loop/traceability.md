...
| S10-001 | 10 | Local helper launches processes/PTYS, worktrees, enforces limits, streams stdout/stderr/events, detects waiting input, redacts logs, records metadata, terminates/culls resources | M1 | `backend/src/lib.rs`; `Sources/CaptureDelegateIPC/IPCClient.swift`; direct-process and event-stream portions implemented on `loop/m1-process-streaming`; run stdin (piped child stdin, send_input/close_stdin with buffered pre-spawn delivery and a stable close boundary) implemented; output redaction and run_metadata implemented (see S10-010); PTY runs implemented (openpty + setsid/TIOCSCTTY controlling terminal, echo-off canonical input, merged stdout/stderr drain, VEOF close with partial-line double-VEOF, redaction/metadata/cancel/timeout parity with pipe runs; opt-in `pty` request field ignored by older backends); input-wait detection implemented (see S10-009); worktree isolation + cleanup implemented (opt-in `worktree_repository` on start_process; private 0700 worktree under a canonicalized managed temp root on a generated `capture-delegate/run-<sanitized-id>-<nonce>` branch; worktree_path/worktree_branch in run_metadata; terminal-first synchronous cleanup with shutdown-blocking guard, bounded kill-on-deadline git invocations, and branch-ownership preflight; runtime failures emit `worktree_failed`); run-termination durability implemented (bounded 5s post-admission write timeout with torn-frame-safe dead-client flag — a non-reading client cannot wedge supervision, leak slots, or stall cleanup; every post-admission failure path emits one best-effort `run_exit` with additive `internal_error` code via a one-shot terminal emitter with centralized drain teardown); stdin-queue byte cap and crash-orphan durable cleanup remain pending | `.loop/verification/m1/process-streaming/full-verification.txt`; `.loop/verification/m1/process-streaming/reviews.txt`; `.loop/verification/m1/run-stdin/verification.txt`; `.loop/verification/m1/run-metadata/verification.txt`; `.loop/verification/m1/pty/verification.txt`; `.loop/verification/m1/worktree/verification.txt`; `.loop/verification/m1/durability/verification.txt` | PR #5 (`9f40024`; both CI runs passed); PR #17 (`67d39f2`; both CI runs passed on both commits; independent Codex review found 2 blockers/2 majors, all confirmed and fixed pre-merge); PR #19 (`9c36a5f`; merged under ADR-007 gate: local verify-m0 parity exit 0 + independent sol review with all confirmed findings fixed); PR #21 (`8e58f7d`; ADR-007 gate: parity exit 0 pre- and post-fix + independent sol review, 0 blockers, both majors dispositioned, both minors fixed); PR #25 (`2a5406b`; ADR-007 gate: parity exit 0 pre- and post-fix + independent sol review — 2 of 3 blockers and 3 of 6 majors confirmed and fixed pre-merge, remainder dispositioned as pre-existing/accepted with rationale on the PR); PR #27 (`36d3705`; ADR-007 gate: parity exit 0 pre- and post-fix + independent sol review — 0 blockers, 1 major confirmed and fixed pre-merge, 3 test-soundness minors fixed with mutation checks) | in_progress |
...
| S10-009 | 10 Responsibilities | Detect agent waits for input | M1 | Implemented: opt-in `input_wait_detect_milliseconds` on start_process; backend emits at most one `run_input_waiting` frame per quiet episode when output is idle past the threshold, the stdin queue is empty, and child CPU time is quiescent (proc_pidinfo PROC_PIDTASKINFO); suppressed while paused (rebaselined on resume); nonblocking emission (writer try_lock); works for pipe and PTY runs; Swift IPCClient parameter + `.inputWaiting` decode case (`backend/src/lib.rs`, `Sources/CaptureDelegateIPC/IPCClient.swift`) | 7 Rust integration tests incl. backpressure-cancel and paused-suppression regressions; Swift decode/wire tests; red evidence vs main; runtime demo; `.loop/verification/m1/input-wait/verification.txt` | PR #23 (`1ad2785`; ADR-007 gate: verify-m0 parity exit 0 pre- and post-fix + independent sol review — 1 blocker/1 major/1 minor, all confirmed and fixed pre-merge) | implemented |
...
| S10-011 | 10 Responsibilities | Terminate cancellation/budget-expired runs and clean temporary files/worktrees | M1 | `backend/src/lib.rs` cancels, kills, and reaps an active producer after output disconnect, kills/reaps direct children on per-run timeout while releasing capacity, and now exposes a versioned `cancel_process` request with an ActiveRuns registry (duplicate run-id rejection, Drop-based release) and typed `cancelled` terminals; `Sources/CaptureDelegateIPC/IPCClient.swift` adds `cancelProcess`; every run now leads its own process group and cancel/timeout/natural-exit/error paths SIGKILL the whole group so descendants cannot outlive a run; SIGTERM/SIGINT shutdown cancels all active runs via an async-signal-safe self-pipe (FD_CLOEXEC, nonblocking write end, pre-bind signal masking), atomically closes run admission during the drain window, removes the still-owned socket, and exits 0; pause_process/resume_process SIGSTOP/SIGCONT the whole group with all pgid/pause transitions and signals serialized under the registry mutex (Swift pauseProcess/resumeProcess mirror cancel); budget expiry and temporary/worktree cleanup remain pending | PR #5 disconnect/inherited-pipe tests; Rust timeout output/order/capacity tests; Rust cancel/duplicate/race integration tests; Swift real-backend timeout and cancel tests; `.loop/verification/m1/run-timeout/verification.txt`; `.loop/verification/m1/run-cancel/verification.txt`; Rust descendant-kill tests (cancel/timeout/natural-exit); `.loop/verification/m1/process-groups/verification.txt`; Rust SIGTERM/SIGINT shutdown tests and shutdown-admission unit test; `.loop/verification/m1/shutdown-cleanup/verification.txt`; Rust group pause/resume, malformed/unknown-pause, and paused-timeout tests plus Swift real-backend pause round-trip; `.loop/verification/m1/pause-resume/verification.txt` | PR #5 (`9f40024`; both CI runs passed); PR #7 (`3058370`; both CI runs passed); PR #9 (`ea02d67`; both CI runs passed; independent Codex review clear, two confirmed findings fixed pre-merge); PR #11 (`8877242`; both CI runs passed; independent Codex review clear, three confirmed findings fixed pre-merge); PR #13 (`af7db55`; both CI runs passed on both commits; independent Codex review found 1 blocker/2 major/1 minor, all fixed pre-merge); PR #15 (`9291f52`; both CI runs passed on both commits; independent Codex review found 1 blocker/1 major/3 minor, all confirmed findings fixed pre-merge) | in_progress |
