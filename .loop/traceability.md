...
| S10-001 | 10 | Local helper launches processes/PTYS, worktrees, enforces limits, streams stdout/stderr/events, detects waiting input, redacts logs, records metadata, terminates/culls resources | M1 | `backend/src/lib.rs`; `Sources/CaptureDelegateIPC/IPCClient.swift`; direct-process and event-stream portions implemented on `loop/m1-process-streaming`; run stdin (piped child stdin, send_input/close_stdin with buffered pre-spawn delivery and a stable close boundary) implemented; output redaction and run_metadata implemented (see S10-010); PTY, worktree, input-wait detection, and durable cleanup remain pending | `.loop/verification/m1/process-streaming/full-verification.txt`; `.loop/verification/m1/process-streaming/reviews.txt`; `.loop/verification/m1/run-stdin/verification.txt`; `.loop/verification/m1/run-metadata/verification.txt` | PR #5 (`9f40024`; both CI runs passed); PR #17 (`67d39f2`; both CI runs passed on both commits; independent Codex review found 2 blockers/2 majors, all confirmed and fixed pre-merge); PR #19 (`9c36a5f`; merged under ADR-007 gate: local verify-m0 parity exit 0 + independent sol review with all confirmed findings fixed) | in_progress |
