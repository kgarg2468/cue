...
|---|---|---|---|---|---|---|---|
| CTL-001 | Loop control | Durable control state records cycle, active milestone, branch, job cadence, expiry, blockers, and summary | M0 | `.loop/state.json` | `python3 -m json.tool .loop/state.json` (passed locally, 2026-07-17) | — | implemented |
| CTL-002 | Loop control | Full end-state roadmap and completion authority are durable | M0 | `.loop/roadmap.md`; `.loop/decisions.md` (ADR-005) | `rg -q 'M14 Full audit' .loop/roadmap.md && rg -q 'ADR-005' .loop/decisions.md` (passed locally, 2026-07-17) | — | implemented |
| M0-FOUNDATION-001 | Approved M0 plan | Swift macOS application and IPC client shell | M0 | `Package.swift`; `Sources/CaptureDelegateApp/`; `Sources/CaptureDelegateIPC/` | `scripts/verify-m0.sh`; `.loop/verification/m0/runtime/app-observation.txt` (passed locally, 2026-07-17) | — | implemented |
| M0-FOUNDATION-002 | Approved M0 plan | Rust local backend service shell | M0 | `backend/src/main.rs`; `backend/src/lib.rs` | `scripts/verify-m0.sh`; `.loop/verification/m0/runtime/backend-liveness.txt` (passed locally, 2026-07-17) | — | implemented |
| M0-FOUNDATION-003 | Approved M0 plan | Versioned bounded Unix-socket health contract | M0 | `backend/src/lib.rs`; `Sources/CaptureDelegateIPC/IPCClient.swift` | `scripts/verify-m0.sh`; `.loop/verification/m0/runtime/health-success.txt`; `incompatible-version.txt` (passed locally, 2026-07-17) | — | implemented |
| M0-FOUNDATION-004 | Approved M0 plan | CI, test, lint, and parity verification scaffold | M0 | `.github/workflows/ci.yml`; `scripts/verify-m0.sh` | `scripts/verify-m0.sh` (passed locally, 2026-07-17) | — | implemented |
| M0-FOUNDATION-005 | Approved M0 plan | Open-source repository baseline | M0 | `README.md`; `LICENSE`; `.gitignore` | `git check-ignore target .build .loop/LOCK` (passed locally, 2026-07-17) | — | implemented |
| M0-FOUNDATION-006 | Approved M0 plan | Durable loop controls and traceability authority | M0 | `.loop/state.json`; `.loop/traceability.md`; `.loop/progress.md`; `.loop/roadmap.md`; `.loop/decisions.md` | `python3 -m json.tool .loop/state.json`; `scripts/verify-m0.sh` (passed locally, 2026-07-17) | — | implemented |
...
| S10-001 | 10 | Local helper launches processes/PTYS, worktrees, enforces limits, streams stdout/stderr/events, detects waiting input, redacts logs, records metadata, terminates/culls resources | M1 | — | — | — | todo |
...
| S10-005 | 10 Responsibilities | Launch child processes and PTYs | M1 | — | — | — | todo |
| S10-006 | 10 Responsibilities | Create isolated Build worktrees | M7 | — | — | — | todo |
| S10-007 | 10 Responsibilities | Enforce local timeout and concurrency limits | M1 | — | — | — | todo |
| S10-008 | 10 Responsibilities | Stream stdout, stderr, and structured events | M1 | — | — | — | todo |
...
| S10-011 | 10 Responsibilities | Terminate cancellation/budget-expired runs and clean temporary files/worktrees | M1 | — | — | — | todo |
