...
| S10-011 | 10 Responsibilities | Terminate cancellation/budget-expired runs and clean temporary files/worktrees | M1 | `backend/src/lib.rs` cancels, kills, and reaps an active producer after output disconnect and releases process capacity; public cancel/budget APIs and temporary/worktree cleanup remain pending | Rust `disconnecting_from_yes_stops_the_child_and_leaves_the_backend_responsive`, `inherited_output_pipes_do_not_delay_the_terminal_frame`; `.loop/verification/m1/process-streaming/full-verification.txt`; independent reviews clear | PR #5 (open) | in_progress |
