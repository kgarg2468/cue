...
| S10-009 | 10 Responsibilities | Detect agent waits for input | M1 | Transport groundwork merged: `send_input`/`close_stdin` requests deliver input to running runs (`backend/src/lib.rs`, `Sources/CaptureDelegateIPC/IPCClient.swift`); detection itself remains pending (expected with PTY support) | Rust cat round-trip/boundary/paused-full-pipe tests; Swift real-backend stdin test; `.loop/verification/m1/run-stdin/verification.txt` | PR #17 (`67d39f2`; both CI runs passed on both commits; independent Codex review found 2 blockers/2 majors, all confirmed and fixed pre-merge) | in_progress |
